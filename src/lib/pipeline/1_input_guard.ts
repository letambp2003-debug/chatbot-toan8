...
export function runInputGuard(question: string): InputGuardResult {
  ...
  // 1. Sanitize text: loại bỏ ký tự điều khiển độc hại, chuẩn hóa khoảng trắng
  let sanitized = question
    .replace(/\0/g, "")
    .replace(/[\u0001-\u0008\u000B\u000C\u000E-\u001F\u007F]/g, "")
    .trim();

  // 2. Giới hạn độ dài tối đa 2000 ký tự cho một câu hỏi
  ...
  // 3. Quét các mẫu Prompt Injection / System Jailbreak
